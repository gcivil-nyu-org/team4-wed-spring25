...
class UserReport(models.Model):
    ...
    user_being_reported = models.ForeignKey(User, on_delete=models.CASCADE, related_name='reports_received')
    ...
    reason = models.TextField(
        _('Optional details'),
        blank=True,
        help_text=_('You can provide specific details about the report here, if necessary.')
    )

    reported_time = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Report {self.report_id} by {self.reporter} against {self.user_being_reported} ({self.get_category_display()})"
